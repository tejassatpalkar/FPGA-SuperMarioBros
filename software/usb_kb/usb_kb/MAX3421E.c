...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	toSend[0] = reg + 2;
	toSend[1] = val;
	ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 2, toSend, 0, garbage, 0);
	if (ret_code < 0) {
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
		...
		ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, length, toSend, 0, garbage, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//psuedocode:
		...
		//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
		//return val
		BYTE spi_cmd_string[5];
		BYTE spi_shit_read[5];
		spi_cmd_string[0] = reg;
		//psuedocode:
		//select MAX3421E (may not be necessary if you are using SPI peripheral)
		//write reg via SPI
		//SPI_0_NUMSLAVES
		int ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, spi_cmd_string, 1, spi_shit_read, 0);
		//read val via SPI
		//read return code from SPI peripheral (see Intel documentation)
		//if return code < 0 print an error
	//	printf("# of bytes read: %d", ret_code);
		BYTE val = spi_shit_read[0];
		if (ret_code < 0){
			printf("error");
			return -1;
		}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	//psuedocode:
		...
		ret_code = alt_avalon_spi_command(SPI_0_BASE, 0, 1, toSend, nbytes, data, 0);
		...
		return data + nbytes; // end of data
}
